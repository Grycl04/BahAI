...
// Confidence threshold (adjusted for better results)
...
async function detectFaceInID(idImageUrl) {
    try {
        ...
        if (errorMsg.includes("INVALID_IMAGE_URL")) {
            return {
                success: false,
                error: "Unable to access ID image. Please try uploading again."
            };
        }
        ...
        if (errorMsg.includes("IMAGE_ERROR")) {
            return {
                success: false,
                error: "ID image format error. Please ensure it's a clear JPG or PNG."
            };
        }
        
        return {
            success: false,
            error: `Face detection failed: ${errorMsg}`
        };
    }
}
...
async function detectFaceInSelfie(selfieImageUrl, retries = 3) {
    try {
        ...
        if (errorMsg.includes("INVALID_IMAGE_URL")) {
            return {
                success: false,
                error: "Unable to access selfie image. Please try uploading again."
            };
        }
...
async function compareFaces(idFaceToken, selfieFaceToken, retries = 3) {
    try {
        ...
        return {
            success: false,
            error: `Face comparison failed: ${errorMsg}`
        };
    }
}
...
async function extractIDData(idImageUrl) {
    try {
        ...
        // OCR is optional, so we don't fail the entire verification
        return {
            success: false,
...
/**
 * Main Cloud Function: Verify KYC Documents with URLs
 */
...
exports.onKYCSubmitted = onDocumentUpdated("users/{userId}", async (event) => {
    ...
    // Check if KYC status changed to pending
...
/**
 * Internal verification function used by Firestore trigger
 */
